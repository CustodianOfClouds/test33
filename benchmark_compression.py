#!/usr/bin/env python3
"""
Benchmark script to compare LRU v2.1 and LFU compression implementations.
...
def benchmark_file(test_name, input_file, alphabet, max_bits, lru_script, lfu_script):
    ...
    # Test LRU v2.1
    print("\n[LRU v2.1]")
    lru_compressed = f"temp_lru_{max_bits}.lzw"
    lru_decompressed = f"temp_lru_{max_bits}_out.txt"

    comp_time = run_compression(lru_script, input_file, lru_compressed, alphabet, max_bits)
    if comp_time is not None:
        compressed_size = get_file_size(lru_compressed)
        decomp_time = run_decompression(lru_script, lru_compressed, lru_decompressed)

        if decomp_time is not None and verify_roundtrip(input_file, lru_decompressed):
            ...
            results['lru'] = {'verified': False}

    # Clean up LRU temp files
    for f in [lru_compressed, lru_decompressed]:
        if os.path.exists(f):
            os.remove(f)
    ...
    # Comparison
    if 'lru' in results and 'lfu' in results and results['lru']['verified'] and results['lfu']['verified']:
        print("\n[Comparison]")
        lru = results['lru']
        lfu = results['lfu']

        size_diff = lfu['compressed_size'] - lru['compressed_size']
        size_diff_pct = (size_diff / lru['compressed_size']) * 100
        comp_speedup = lru['compression_time'] / lfu['compression_time']
        decomp_speedup = lru['decompression_time'] / lfu['decompression_time']

        print(f"  Size difference: {format_size(abs(size_diff))} ({'LFU smaller' if size_diff < 0 else 'LRU smaller'})")
        print(f"  Size difference %: {abs(size_diff_pct):.2f}% ({'LFU' if size_diff < 0 else 'LRU'})")
        print(f"  Compression speedup: {comp_speedup:.2f}x ({'LFU faster' if comp_speedup > 1 else 'LRU faster'})")
        print(f"  Decompression speedup: {decomp_speedup:.2f}x ({'LFU faster' if decomp_speedup > 1 else 'LRU faster'})")

    return results

def main():
    lru_script = 'LRU-Eviction/LZW-LRU-Optimizedv2.1.py'
    ...
    for result in all_results:
        if 'lru' in result and 'lfu' in result:
            if result['lru'].get('verified') and result['lfu'].get('verified'):
                if result['lru']['compressed_size'] < result['lfu']['compressed_size']:
                    lru_wins_ratio += 1
                else:
                    lfu_wins_ratio += 1

                if result['lru']['compression_time'] < result['lfu']['compression_time']:
                    lru_wins_speed += 1
                else:
                    lfu_wins_speed += 1

    total_tests = lru_wins_ratio + lfu_wins_ratio
    print(f"\nCompression Ratio:")
    print(f"  LRU wins: {lru_wins_ratio}/{total_tests} ({lru_wins_ratio/total_tests*100:.1f}%)")
    print(f"  LFU wins: {lfu_wins_ratio}/{total_tests} ({lfu_wins_ratio/total_tests*100:.1f}%)")

    print(f"\nCompression Speed:")
    print(f"  LRU faster: {lru_wins_speed}/{total_tests} ({lru_wins_speed/total_tests*100:.1f}%)")
